...
| **WP-V90** **`P0`** | **La suite de jest NO es determinista — y el gate R7-V se apoya en ella** (hallazgo convergente de **tres** WPs independientes de la ola 3: V66, V71 y V23, este último con la medida). Cinco corridas seguidas sin tocar nada: `5, 5, 5, 6, 5` rojos; con la máquina cargada, hasta **8 fallos y 3 suites**. El sexto tiene nombre y causa: `tests/basic.test.ts:23 · should measure performance`, **aserción de reloj de pared** (`duration < 100 ms`); el otro sospechoso es `tests/performance/serviceStartup.test.ts` (`setTimeout(10)` con margen de 90 ms). Los ficheros que flapean son **byte a byte los de la base**: no lo causa ningún WP. **Consecuencia para el gate**: `R7-V` y cualquier CA que compare **conteos exactos** de jest dará **falsos rojos** — y, peor, un rojo real puede confundirse con flapeo. Mientras no se cierre, la fórmula honesta es «idéntico **en el conjunto determinista**», con los rojos **por nombre y causa**, nunca por número | las aserciones de reloj de pared se sustituyen por relojes falsos o se marcan como no-deterministas · 10 corridas seguidas dan el mismo conjunto **por nombre** · el gate compara nombres, no cardinales |
