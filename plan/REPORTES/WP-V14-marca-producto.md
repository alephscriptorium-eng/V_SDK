...
| **RES-7** | `docs/GUIA-PRUEBA-v1.md` ⛔ *(cita rancia: retirada por V15 (`808be04`); hoy `docs/GUIA-PRUEBA-v2.md`. Se conserva porque era cierta al escribirse)* está escrita entera sobre «Zigurat» y sobre las claves `zigurat.*` (10 líneas). No viaja en el `.vsix` (`.vscodeignore:27`), así que no es CA-1, pero es el documento con el que el vigía-S valida | `docs/GUIA-PRUEBA-v1.md` | **V15** (cuando cambien las claves, la guía deja de funcionar) |
