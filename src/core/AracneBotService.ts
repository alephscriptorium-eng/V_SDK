...
export class AracneBotService {
    ...
    /** true si falta aleph0.ciudad.* / socketUrl (hostil-omite). */
    ...
    public initialize(config?: Partial<AracneBotConfig>): void {
        ...
        if (!this.config.socketUrl) {
            ...
            console.warn(
                `[AracneBot] ${ZIGURAT_PENDING} aleph0.ciudad.baseUrl (o host+port) no configurado — sin cliente Socket.IO`
            );
            return;
        }
    ...
    public connect(): void {
        if (this.pending || !this.client) {
            console.warn(
                `[AracneBot] ${ZIGURAT_PENDING} sin mesh configurado. Configure aleph0.ciudad.baseUrl (o host+port).`
            );
            return;
        }
